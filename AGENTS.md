...
Cloudflare 会对 `/assets/*` 设置较长的浏览器缓存。新增或替换 CSS、JavaScript、图片和 SVG 等静态资源时，必须使用能区分版本的文件名，例如 `style-v20260705.css`、`profile-v2.webp`，并同步更新 HTML 引用，避免访问者长期拿到旧文件。
...
Cloudflare 缓存策略应保持保守：`/assets/*` 的 Browser Cache TTL 为 1 month；主页和 HTML 页面不要长缓存，Browser Cache TTL 控制在 30 min；版本化 CV PDF（`KaiWU_CV*.pdf` 或 `KaiWu_CV*.pdf`）可设置 Browser Cache TTL 为 1 month。不要对未版本化的公开资源启用长浏览器缓存。
